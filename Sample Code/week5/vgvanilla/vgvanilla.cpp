...
double   sig=.3, nu=.25, theta=-.3;
double   lambda_n, lambda_p;
double   omega;

double   T=1.0;
double   S0=100.0, K=100.0; 
double   rfr=.066, div=.012;
double   SMIN = 1, SMAX = 6000.0;

int      N=1000,M=2000;


FILE * fout ;
fout=fopen("prices.dat","w");    
	 ...
	 for (j = M-1; j >= 0; j--) {
		 ...
		 // Boundary Conditions 
		 W[0]=K*exp(-r[j]*(T-t[j]))-exp(x[0])*exp(-q[j]*(T-t[j]));
		 W[N]=0;
         // 
		 for (i=1;i<=N-1;i++){
			 B[i]=W[i];
		 ...
		 for (i = 1; i <=N-1; i++){
			 if (i==1){
				 D[i] = 1 + r[j]*Dt + Bp + Bn + c*(expint((N-i)*lambda_p*Dx)+expint(i*lambda_n*Dx));
		         U[i] = -A[j] - Bp;
				 for (k=1;k<=N-i-1;k++)
					 B[i] += c*((W[i+k]-W[i])-k*(W[i+k+1]-W[i+k]))*(ei_p[k]-ei_p[k+1]) + cp*(W[i+k+1]-W[i+k])*(exp_p[k]-exp_p[k+1]);
				 B[i] += c*(K*exp(-r[j]*(T-t[j]))*expint(i*Dx*lambda_n) - exp(-q[j]*(T-t[j]))*exp(x[i])*expint(i*Dx*(lambda_n+1)));
				 B[i] += -(A[j] - Bn)*( K*exp(-r[j]*(T-t[j])) - exp(x[0])*exp(-q[j]*(T-t[j])) );  // Boundary Condition Effect 
